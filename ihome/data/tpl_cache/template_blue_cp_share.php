<?php if(!defined('iBUAA')) exit('Access Denied');?><?php subtplcheck('template/blue/cp_share|template/blue/header|template/blue/cp_topic_menu|template/blue/space_share_li|template/blue/footer|template/blue/space_topic_inc', '1381749150', 'template/blue/cp_share');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
<!DOCTYPE>
<html>
<head>
...
<?php if($_SGLOBAL['inajax'] && $_GET['type']!='view') { ?>
...
<div class="c_form">
<form id="shareform" name="shareform" action="cp.php?ac=share&type=link&inspace=<?php echo $inspace; ?>" method="post">
...
<form method="post" id="shareform_<?=$id?>" name="shareform_<?=$id?>" action="cp.php?ac=share&type=<?=$type?>&id=<?=$id?>&inspace=<?=$inspace?>">
<table>
<tr>
<td>分享说明:
...
<textarea id="general" name="general" style="width:380px;" rows="3" onkeydown="ctrlEnter(event, 'sharesubmit_btn')" onkeyup="showPreview(this.value, 'quote')" onfocus="at()"></textarea>
</td>
</tr>
<tr>
<td>
...
<?php } ?>
<?php if($arr['body_data']['videourl']) { ?>
<label><input type="checkbox" name="withcomment" value="withcomment" checked />同时评论</label>
<?php } ?>
</td>
</tr>
</table>
