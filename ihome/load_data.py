...
def load_job(conn):
    cursor = conn.cursor()
    cursor.execute("select j.id,j.uid,j.title,jc.endtime,jc.description,jc.content from ihome_job j right join ihome_job_content_3 jc on j.id=jc.jobid where j.type=3")
    rows = cursor.fetchall()
    ...
    for row in rows:
        ...
        data['uid'] = str(row[1])
        data['jobtitle'] = row[2]
        data['jobendtime'] = str(row[3])
        data['jobdescription'] = row[4]
        data['jobcontent'] = row[5]
        data['id'] = 'jobid_%d' % row[0]
        data['type'] = 'job'
        datas.append(data)
        datastr = json.dumps(datas)
        #print datastr
...
def load_arrangement(conn):
    cursor = conn.cursor()
    cursor.execute("select a.arrangementid, a.uid, a.subject, a.starttime, a.message from ihome_arrangement a")
    rows = cursor.fetchall()
    ...
    for row in rows:
        ...
        datas = []
        data = {}
        data['arrangementid'] = row[0]
        data['uid'] = str(row[1])
        data['arrangementsubject'] = row[2]
        data['arrangementstarttime'] = str(row[3])
        data['arrangementmessage'] = row[4]
        data['id'] = 'arrangementid_%d' % row[0]
...
def load_complain(conn):
    cursor = conn.cursor()
    cursor.execute("select a.doid, a.uid, a.message, a.dateline from ihome_complain a")
    rows = cursor.fetchall()
    #print "all", len(rows)
    i = 0
    for row in rows:
