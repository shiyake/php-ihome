...
*/
	...
	if($value[type]=='arrangement'){
		...
		$query1 = $_SGLOBAL['db']->query("SELECT * from ".tname('arrangement')." where  $wheresql  ");
		...
		foreach($list as $value){
					$result[] = array('flag'=>'success','blog_authorpic'=>avatar($value['uid'],small),'blog_title'=>$value['subject'],'blog_author'=>$_SN[$value[uid]],'blog_authorid'=>$value[uid],'blog_image'=>$value[pic],'blog_text'=>$value['message'],'blog_readnum'=>$value['viewnum'],'blog_replynum'=>$value['replynum'],'blog_image'=>$value['pic'],'blog_noreply'=>'','blog_time'=>$value[dateline]);
				}
