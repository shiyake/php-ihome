...
*/
	...
	*/
	...
	if(trim($Message)==null){
		...
		//$Message = rawurldecode(getstr($Message, 1000, 1, 1, 1, 2));
	...
	//替换表情
	$Message = preg_replace("/\[em:(\d+):]/is", "<img src=\"image/face/\\.gif\" class=\"face\">", $Message);
	$Message = preg_replace("/\<br.*?\>/is", ' ', $Message);
	
	$arr = array(
	"username" => getstr($username, 15, 1, 1, 1),
	"message" => saddslashes(serialize(sstripslashes($Message))),
	"uid" => intval($userid),
	"replynum"=>0,
	"mood"=>0,
