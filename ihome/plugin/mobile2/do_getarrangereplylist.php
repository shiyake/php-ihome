...
*/
	...
	$result = array();
	$replylist = array();
	$wheresql = "id=$arrangementid and idtype='arrangementid' ";
		...
		if(preg_match_all("#image\/face\/(\d+)\.gif#i",$TmpString, $matchface, PREG_SET_ORDER)){
			...
			$value['message'] = str_replace($TmpFace, $newface, $value['message']);
				...
				$result[] = array('arrange_authorpic'=>avatar($value['authorid'],middle),'arrange_id'=>$value['cid'],'arrange_userid'=>$value['authorid'],'arrange_username'=>$_SN[$value['authorid']],'arrange_text'=>$value['message'],'arrange_time'=>$value['dateline']);
			}
		}
	}
