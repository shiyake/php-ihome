...
*/
	...
	$wheresql .= " and (body_template not like '%{eventname}%' and body_template not like '%{link}%' and body_template not like '%{summary}%' and body_template not like '%{time}%' and body_template not like '%{mtag}%' and body_template not like '%{spacenote}%' and  body_template not like '%{option}%') and icon in ('doing','blog','arrangement','thread','share','album')";
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		...
		//去除空格符号
		preg_match_all("#&nbsp;#U",$value['body_template'], $matches, PREG_SET_ORDER);
	...
	//echo "----matchstring: $MatchString----tmpstring: $TmpString----username: $HrefString\n";
		...
		//去除<span></span>标签
		preg_match_all("#[<]span[>](.*)[<][\][/]span[>]#U",$value['body_template'], $matches, PREG_SET_ORDER);
	...
	//echo "----matchstring: $MatchString----tmpstring: $TmpString----username: $HrefString\n";
		...
		//去除</span>标签
		preg_match_all("#[<][\][/]span[>]#U",$value['body_template'], $matches, PREG_SET_ORDER);
...
//将公告中的图片进行绝对路径化。 by xuxing start<img src=\"image\/face\/24.gif\" class=\"face\">
	...
	//echo "----matchstring: $MatchString----tmpstring: $TmpString----username: $HrefString\n";
	 $TmpFace = $item[0];
		...
		$result[] = array('user_thumbpic'=>avatar($value[uid],small),
		'feed_uid'=>$value[uid],
		'feed_name'=>$_SN[$value[uid]],
		'feed_id'=>$value[feedid],
		'feed_type'=>in_array($value['icon'], array('blog','album','share','poll','thread','event','doing','arrangement'))?$value['icon']:'',
		'feed_target_id'=>$value['id'],
		'feed_comment_flag'=>in_array($value['idtype'], array('blogid','picid','albumid','sid','pid','eventid','doid'))?'1':'0',
		'feed_image_2'=>$value[image_2],
		'feed_image_3'=>$value[image_3],
		'feed_image_4'=>$value[image_4],
		'feed_detail'=>$value['body_template'],
		'feed_quote'=>$value[body_general],
		'creat_at'=>$value[dateline],
		'feed_image_1'=>$value[image_1]);
	}
	...
	//$endtime = time();
	/*
	$str ="接收数据时间:".$starttime.","."数据库开始时间:".$DBstarttime.","."数据库结束时间:".$DBendtime.","."结束时间:".$endtime."\r\n";
	
	$k=fopen(S_ROOT."/data/log/mobile2_log.txt","a+");//此处用a+，读写方式打开，将文件指针指向文件末尾。如果文件不存在则尝试创建之
	fwrite($k,$str);
	*/
	exit();
