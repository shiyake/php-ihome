...
//$userid=trim($_POST["sess_userid"]);
	...
	$query = $_SGLOBAL['db']->query("SELECT uid,feedid,icon,id,idtype,image_1,body_template,body_general,dateline FROM ".tname('feed')." USE INDEX(dateline) WHERE ".$wheresql." ORDER BY dateline DESC LIMIT ".$start.",".$perpage);
echo 'DB accept:'.time().'----';
...
//将公告中的图片进行绝对路径化。 by xuxing start<img src=\"image\/face\/24.gif\" class=\"face\">
	...
	//echo "----matchstring: $MatchString----tmpstring: $TmpString----username: $HrefString\n";
	 $TmpFace = $item[0];
		...
		$result[] = array('user_thumbpic'=>avatar($value[uid],middle),
						'user_uid'=>$value[uid],
						...
						'message'=>$value['body_template'],
						'title'=>$value[body_general],
						'creat_at'=>$value[dateline],
						'feed_image'=>$value[image_1]);
	}
