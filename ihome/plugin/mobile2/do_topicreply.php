...
*/
	...
	$wheresql = "tid =".$id. " and isthread=0";
	$query = $_SGLOBAL['db']->query("SELECT tagid,tid,uid,message,dateline,pid FROM  ".tname('post')."  WHERE ".$wheresql." order by dateline asc LIMIT  $start,$perpage");
	...
	if($topicreplylist){
		foreach($topicreplylist as $value){
			$result[] = array('topic_groupid'=>$value['tagid'],'topic_id'=>$value[tid],'reply_userpic'=>avatar($value[uid],middle),'reply_uid'=>$value['uid'],'reply_pid'=>$value[pid],'reply_user'=>$_SN[$value[uid]],'reply_text'=>$value[message],'reply_time'=>$value[dateline]);
		}
	}
