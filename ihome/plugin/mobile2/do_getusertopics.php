...
*/
	...
	$query = $_SGLOBAL['db']->query("SELECT main.tid,main.dateline,main.subject,main.tagid,mt.tagname,mt.fieldid,main.replynum  FROM ".tname('thread')." main 
	...
	foreach($topiclist as $value){
		$result[] = array('topic_id'=>$value[tid],'create_at'=>$value[dateline],'topic_title'=>$value[subject],
		'tagid'=>$value[tagid],'tagname'=>$value[tagname],'topic_group'=>$value[mygroupname],
		'topic_replycount'=>$value[replynum],'topic_readcount'=>$value[viewnum]);
	}
