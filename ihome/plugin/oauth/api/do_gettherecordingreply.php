...
*/
	...
	if($topiclist){
		...
		if(preg_match_all("#image\/face\/(\d+)\.gif#i",$TmpString, $matchface, PREG_SET_ORDER)){
			...
			$result[] = array('news_userid'=>$value[uid],'news_username'=>$_SN[$value[uid]],'news_userpic'=>avatar($value[uid],small),'news_id'=>$value[id],'news_text'=>$value[message],'news_time'=>$value[dateline]);
		}
	}
