...
/************************测试时为0,正式环境时应该为$_SGLOBAL['timestamp'] - 900.*******************************/
$RightNow = 0;
...
if(empty($op)) {
	if($WallId) {
		...
		if($uid == $UserId || $uid == 10000 || $uid == 1) {
			$Query = $_SGLOBAL['db']->query("select * from ".tname("wallfield")." where wallid=".$WallId." and pass >= 1 and timeline > ".$RightNow." AND display < 1 order by checktime ASC limit 3 ");//timeline是发布时间
...
if($op =='fetchOne') {
	...
	/*
	//热度排行优先，仅显示一次。热度加1的，则再显示一次。热度加2的，显示两次。以此类推。
	if($uid == $UserId || $uid == 10000 || $uid == 1) {
		$Query = $_SGLOBAL['db']->query("select * from ".tname("wallfield")." where wallid=".$WallId." and pass >= 1 and timeline > ".$RightNow." and display = 1  and hot = 1 limit 1");
		if($Value = $_SGLOBAL['db']->fetch_array($Query)) {
			inserttable('walluser', array('wfid' =>$Value['id'], 'userid' =>$uid), 1, true);
			$floorid =  $_SGLOBAL['db']->insert_id();
			$Value['floorid'] = $floorid;
			updatetable('wallfield', array('display' => 2, 'now' =>$_SGLOBAL['timestamp'], 'floorid' =>$Value['floorid']), array('id' =>$Value['id']));
			realname_set($Value[uid]);
			$WallRecList[] = $Value;
			realname_get();
	...
	*/
	//审核时间。取消审核时，把display=0和pass都取消。
	if($uid == $UserId || $uid == 10000 || $uid == 1) {
		$Query = $_SGLOBAL['db']->query("select * from ".tname("wallfield")." where wallid=".$WallId." and pass >= 1 and timeline > ".$RightNow." and display < 1 order by checktime ASC limit 1");
		if($Value = $_SGLOBAL['db']->fetch_array($Query)) {
			inserttable('walluser', array('wfid' =>$Value['id'], 'userid' =>$uid), 1, true);
			$floorid =  $_SGLOBAL['db']->insert_id();
			$Value['floorid'] = $floorid;
