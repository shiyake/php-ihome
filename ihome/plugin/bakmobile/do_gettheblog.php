...
//制作封面缩略图 start
	if(empty($blog[pic])){
			  $blog[pic]='';	
			}else if(preg_match("/attachment\/(.*)/",$blog['pic'],$matches) && (file_exists($_SC['attachdir'].'./'.$matches[1].'.thumb.jpg') || file_exists($_SC['attachdir'].'./'.$matches[1]))){
				//echo file_exists($_SC['attachdir'].'./'.$matches[1]);exit();
				if (file_exists($_SC['attachdir'].'./'.$matches[1].'.thumb.jpg')) {
					$blog['pic'] = $_SCONFIG[siteallurl].'attachment/'.$matches[1].'.thumb.jpg';
				}else{
					$blog['pic'] = $_SCONFIG[siteallurl].'attachment/'.$matches[1];
				}
			  ...
			  //$value['image_1']='（图片来自外网，耗费流量较多，请使用浏览器查看）';
				$dir1 = gmdate('Ym',$blog['dateline']);
				$dir2 = gmdate('d',$blog['dateline']);
				$filename = basename($blog['pic']);
				...
				if(file_exists($_SC['attachdir'].'./'.$filepath.'.thumb.jpg')) {
					$blog['pic'] = $_SCONFIG[siteallurl].'attachment/'.$filepath.'.thumb.jpg';
				}else{
					if($foreignthumb = makemobilethumb($blog['pic'], $filename, $dir1, $dir2)){
						$blog['pic'] = $_SCONFIG[siteallurl].'attachment/'.$foreignthumb;					
					}else{
						$blog['pic']='';
					}
...
//将日志中的图片进行绝对路径化。 start
	...
	foreach($matche as $item){
		...
		//开始处理图片		
	if(preg_match("/attachment\/(.*)/",$TmpString,$matches) && (file_exists($_SC['attachdir'].'./'.$matches[1].'.thumb.jpg') || file_exists($_SC['attachdir'].'./'.$matches[1]))){
		if (file_exists($_SC['attachdir'].'./'.$matches[1].'.thumb.jpg')) {
		//echo 11111;
			$HrefString = $_SCONFIG[siteallurl].'attachment/.'.$matches[1].'.thumb.jpg';
		}else{
			$HrefString = $_SCONFIG[siteallurl].'attachment/.'.$matches[1];
		}
	}else{
	//echo 2222;
		$dir1 = gmdate('Ym',$blog['dateline']);
		$dir2 = gmdate('d',$blog['dateline']);
		$filename = basename($TmpString);
		$filepath = getforeignpicpath($filename, $dir1, $dir2, true);
		//echo $_SC['attachdir'].'./'.$filepath;exit();
		if(file_exists($_SC['attachdir'].'./'.$filepath.'.thumb.jpg')) {
			$HrefString = $_SCONFIG[siteallurl].'attachment/'.$filepath.'.thumb.jpg';
		}else{
			if($foreignthumb = makemobilethumb($TmpString, $filename, $dir1, $dir2)){
				$HrefString = $_SCONFIG[siteallurl].'attachment/'.$foreignthumb;					
			}else{
				$HrefString='';
			}
		}
