...
function iauth_verify( $url ='' ){
    ...
    $params['uid']=$uid;
    if(md5(CoString($params).'&'.IAUTH_RP_ID.'&'.IAUTH_RP_SECRET)!=$sig) die('请求校验失败  sig not match');
    return $uid;
    }
...
function GetHeaderParams(){
    ...
    if (empty($headers['Authorization'])) die('needs header Authorization');
