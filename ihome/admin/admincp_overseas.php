...
$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname("spaceforeign")." AS a LEFT JOIN ".tname("space")." AS b ON a.uid=b.uid LEFT JOIN ".tname("forgienCreate")." AS c on b.username=c.username WHERE a.cer=0");
while($res = $_SGLOBAL['db']->fetch_array($query))	{
	$apply[] = $res ;
}
$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname("spaceforeign")." AS a LEFT JOIN ".tname("space")." AS b on a.uid=b.uid LEFT JOIN ".tname("forgienCreate")." AS c on b.username=c.username WHERE a.cer=1");
while($res = $_SGLOBAL['db']->fetch_array($query))	{
	$pass[] = $res ;
}
if("change" == $_GET['do'])	{
