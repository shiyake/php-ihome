...
/*	[iBUAA] (C)2012-2111 BUAANIC . 
	...
	Last Time : 2012年9月10日11:39:45
	这个得放在plugin里面,同时,把诸多东西改正过来
*/
...
if(($thevalue['total'] - $thevalue['ctotal'] ) <1){
...
if($_POST){
	...
	$arr = array(
		...
		"tel" => ismobile(strip_tags($_POST['tel'])),
	...
	feed_add(
		...
		'{actor} 兑换了 [{gift}]',
		array(
			'actor' => "<a href=\"space.php?uid=$uid\">{$_SN[$uid]}</a>",
			'gift' =>"<a href=\"jifen.php?ac=show&id={$lpid}\">{$thevalue[name]}</a>"),
		'{summay}',
		array(
			'summay' =>$summay),
		'',
