...
function setMenuPosition(showid, offset) {
	...
	if(showobj) {
		...
		if(offset != -1&&offset != -2&&offset != -3) {
			...
			menuobj.style.top = Math.max(divtop, 100) + 'px';
		} else if(offset==-2)	{
			menuobj.style.left = (showobj.X + menuobj.w > document.body.clientWidth) && (showobj.X + showobj.w - menuobj.w >= 0) ? showobj.X + showobj.w - menuobj.w -180 + 'px' : showobj.X -180 + 'px' ;
			menuobj.style.top="56px";
			menuobj.style.position="fixed";
		}
		else if(offset==-3)	{
			menuobj.style.left="987px";
			menuobj.style.top="1px";
		}
