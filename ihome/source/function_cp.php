...
function uploadFile($FILE) {
	...
	{
		return cplang('the_file_is_too_big');
	}
	...
	if(@copy($tmp_name, $new_name)) {
		...
		return cplang('mobile_attachment_temporary_failure');
	}
	...
	$setarr['aid'] = inserttable('attach', $setarr, 1);
...
function pic_save($FILE, $albumid, $title, $topicid=0) {
    ...
    $setarr['thumbpath'] = $thumbpath;
    $setarr['new_filepath'] = $new_name;
	return $setarr;
}
...
//诉求通知
function notification_complain_add($uid, $type, $note, $authorid=0, $author='', $returnid=0) {
	global $_SGLOBAL;
...
//更新双向好友状态
function friend_update($uid, $username, $fuid, $fusername, $op='add', $gid=0, $public=0) {
	global $_SGLOBAL, $_SCONFIG;
	...
	//缓存
	if($op=='ignore' && $public==1){
		friend_cache($uid);
		friend_cache($fuid,$uid);//it is a public page, then delete it from the user's feedfriends.
...
function friend_cache($uid,$puid=0) {
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		if($value['fuid']) {
			$newfriendlist[] = $value['fuid'];//存储好友数组by xuxing 2012-09-21
	...
	while($value = $_SGLOBAL['db']->fetch_array($query)){
		...
		$publicpageat[] = $value; //存储公共主页数组信息by xuxing 2012-10-20
	}
	//取出现有的feedfriend
	...
	if($value = $_SGLOBAL['db']->fetch_array($query)){
		...
		$newfriendlist = array_flip(array_flip($newfriendlist));//唯一化数组
...
//处理@好友的定位功能 by xuxing 2012-10-1
