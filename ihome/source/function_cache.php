...
function jifen_lb_cache(){
    global $_SGLOBAL;

	$_SGLOBAL['jifen_lb'] = array();
	// 从数据库获取
	$query = $_SGLOBAL['db']->query("SELECT * FROM " . tname("jifen_lb") . " ORDER BY displayorder");
	while($value = $_SGLOBAL['db']->fetch_array($query)){
		if(!$value['pic']) {
			$value['pic'] = "jifen/images/default.gif";
		}
	    $_SGLOBAL['jifen_lb'][$value['id']] = $value;
	}
	cache_write('jifen_lb', "_SGLOBAL['jifen_lb']", $_SGLOBAL['jifen_lb']);
}
