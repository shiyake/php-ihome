...
$log = Logger::getLogger("quickregister");
...
if($id) {
	...
	$list = array();
	$postnum = $start;
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('post')." WHERE $psql tid='$thread[tid]' ORDER BY dateline DESC LIMIT $start,$perpage");
    ...
    $querythread = $_SGLOBAL['db']->query("SELECT * FROM ".tname('post')." WHERE tid='$thread[tid]' AND isthread='1'");
    $valuethread = $_SGLOBAL['db']->fetch_array($querythread);
    $thread['content'] = $valuethread;
