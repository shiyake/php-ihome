...
jq(function() {
	...
	for(var i = 0; i < rows; i++) {
		...
		for(var j = 0; j < COLS; j++) {
			var index = i * 6 + j + 1;
			FACE_TPL += '<td class="face_cell"><img data-index="' + index + '" src="image/face/' + index + '.gif"></img></td>';
		}
		FACE_TPL += "</tr>";
	}
	FACE_TPL += "</table>";

	function popoverFace(jqEl, getId, show) {
		if (jqEl.data('bs.popover')) {
			return;
		}
		...
		jqEl.popover({
			content: function() {
				var face_content = jq(FACE_TPL);
				face_content.on('click', 'img', function() {
					insertFace(jq(this).data('index'), id);
				});
				return face_content;
			},
			html: true,
	...
	jq(document).on('click', '.fuck_facein', function() {
		popoverFace(jq(this), function(jqEl) {
			return "feedmessage_" + jqEl.attr("id").replace("face_", "");
		}, true);
	});
	jq(document).on('click', '.do_facein', function() {
		popoverFace(jq(this), function(jqEl) {
			return "do_message_" + jqEl.attr("id").replace("do_face_", "");
		}, true);
	});

	popoverFace(jq(".facein"), function(jqEl) {
		return "message";
	});

	//jq(".facein").popover("hide");
});
