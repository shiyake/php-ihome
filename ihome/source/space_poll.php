...
if($pid) {
	...
	foreach($option as $key => $value) {
		if($value['votenum'] && $allvote) {
			...
			$value['width'] = round($value['percent']*190);
	...
	if($isfriend) {
		...
		if($count) {
			...
			$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('comment')." WHERE $csql id='$pid' AND idtype='pid' ORDER BY dateline desc LIMIT $start,$perpage");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
