	...
	if($res = $_SGLOBAL['db']->fetch_array($query))	{
		...
		try {
			...
			$setarr = array(
				...
				'authorid' => $newuid,
				'author' => $name,
				'note' => "$birthday,$academy,".$startyear."级,来自$country,$school的同学".'向您发起了认证请求<br/><a href="space.php?do=friend&view=confirm&uid=%27'.$newuid.'%27&type=overseas">通过请求</a><span class="pipe">|</span><a href="space.php?do=friend&view=refuse&uid=%27'.$newuid.'%27&type=overseas">忽略</a>',
