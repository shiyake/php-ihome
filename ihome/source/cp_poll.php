...
if(submitcheck('pollsubmit')) {
	...
	foreach($_POST['option'] as $key => $val) {
		$option = getstr(trim($val), 180, 1, 1, 1);
	...
	$minchoice = $_POST['minchoice'] <= $maxchoice ? intval($_POST['minchoice']) : $maxchoice;
	$expiration = 0;
	if($_POST['expiration']) {
	...
	foreach($newoption as $key => $value) {
		...
		$value = preg_replace("/\<a\s+href\=([^\>]+?)\>/i", '<a href=\\1 target=_blank>', $value);
		$optionarr[] = "('$pid', '$value')";
	}
...
if($op == 'addopt') {
	...
	if(submitcheck('addopt')) {
		$newoption = getstr(trim($_POST['newoption']), 180, 1, 1, 1);
		$newoption = preg_replace(array(
				"/\&lt\;/i",
				"/\&gt\;/i"
			), array(
				'<',
		...
		$newoption = preg_replace("/\<a\s+href\=([^\>]+?)\>/i", '<a href=\\1 target=_blank>', $newoption);
	...
	if(submitcheck('votesubmit')) {
		...
		$count = $list = $optionarr = $setarr = array();
		$count = $_POST['option'];
		if (count($count) > $poll['maxchoice']) {
			showmessage('above_the_maxchoice');
		}
		if (count($count) < $poll['minchoice']) {
			showmessage('below_the_minchoice');
		}
		foreach($_POST['option'] as $key => $val) {
