...
if($id) {
	...
	//处理查询
	$f_index = '';
	if($_GET['view'] == 'freshman') {
		//新生须知
		$actives = array('freshman'=>' class="active"');
		$theurl = "space.php?uid=$space[uid]&do=$do&view=freshman";
		//关键词
		$KeyWord = "新生须知";
		$wheresql = " binary classname='".$KeyWord."' ";
		
		$count = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('blog')." b, ".tname('class')." c, ".tname('space')." s WHERE b.classid=c.classid and c.uid=s.uid and s.groupid=3 and $wheresql"),0);
		if(count){
			$query = $_SGLOBAL['db']->query("SELECT bf.message, bf.target_ids, bf.magiccolor, b.* FROM ".tname('blog')." b,  
				 ".tname('blogfield')." bf, ".tname('space')." s where bf.blogid=b.blogid and 
				 b.classid in (select classid from ".tname('class')." where $wheresql) and 
				 b.uid=s.uid and s.groupid=3
				ORDER BY b.dateline DESC LIMIT $start,$perpage");
		}
		/*showmessage("SELECT bf.message, bf.target_ids, bf.magiccolor, b.* FROM ".tname('blog')." b,  
				 ".tname('blogfield')." bf, ".tname('space')." s where bf.blogid=b.blogid and 
				 b.classid in (select classid from ".tname('class')." where $wheresql) and 
				 bf.uid=b.uid=s.uid and s.groupid=3 
				ORDER BY b.dateline DESC LIMIT $start,$perpage");*/
