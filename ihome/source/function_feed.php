	...
	switch ($idtype) {
		...
		case 'pid':
			...
			if($value = $_SGLOBAL['db']->fetch_array($query)) {
				...
				$setarr['body_template'] = '<a href="{url}">{subject}</a>{option}';
                ...
                while ($opt = $_SGLOBAL['db']->fetch_array($opquery)) {
					$optionstr .= '<div><input type="'.($value['maxchoice'] > 1 ? 'checkbox' : 'radio').'" disabled name="poll_'.$opt['oid'].'"/>'.$opt['option'].'</div>';
				}
				$setarr['body_data'] = array(
					'url' => $url,
		...
		case 'videoid':
			...
			if($value = $_SGLOBAL['db']->fetch_array($query)) {
				...
				//$url = 'attachment/'.$value['filepath'];
				$setarr['title_template'] = cplang('upload_a_video');
				$setarr['title_data'] = array(
					'title' => "<a href=\"plugin.php?pluginid=video&ac=view&vid=$id\">".$value['title']."</a>"
				);
				//$setarr['body_template'] = cplang('video_info');
				$setarr['body_data'] = array(
					'video_origin' => 'yes',
					'desc' => $value['desc'],
					'videourl' => $url
		...
		case 'appsid':
			//app的基本信息
			$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('apps')." WHERE id='$id' OR iauth_id='$id'");
			if($value = $_SGLOBAL['db']->fetch_array($query)) {
				...
				$setarr['idtype'] = $idtype;
				$setarr['uid'] = $value['uid'];
				$setarr['username'] = 'yangdali';
				$setarr['dateline'] = time();
				//详细
				$url = "plugin.php?pluginid=apps&ac=detail&appsid=$value[id]";
				$value['logo'] = $value['logo'] ? $value['logo'] : 'plugin/apps/images/app.gif';
				$setarr['image_1'] = pic_get($value['logo'],'','');
				$setarr['image_1_link'] = $url;
				
				$setarr['title_template'] = '{actor} '.cplang('use_new_app');
				$setarr['title_data'] = array(
					'title' => "<a href=\"plugin.php?pluginid=apps&ac=detail&appsid=$value[id]\">".$value['name']."</a>"
				);
				$setarr['body_template'] = '{title}';
				$setarr['body_data'] = array('title' => $value['name']);
