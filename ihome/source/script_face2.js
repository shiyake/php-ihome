...
function textCounter(obj, showid, maxlimit) {
	var msg = obj.value;
	var len = strLen(msg.replace(/\s/g,''));
	if(len > 0) {
		$("publisher_submit").parentNode.className='postBtnBg';
	} else {
		$("publisher_submit").parentNode.className='postBtnBg bgColorA_No';
	}
	var showobj = $(showid);
	if(len > maxlimit) {
...
function strLen(str) {
	...
	for(var i = 0; i < str.length; i++) {
		len += str.charCodeAt(i) < 0 || str.charCodeAt(i) > 255 ? (charset == "utf-8" ? 3 : 1) : 0.5;
	}
	return parseInt(len);
}