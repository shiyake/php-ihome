...
$ComplainQuery = $_SGLOBAL['db']->query("SELECT * FROM ".tname('complain')." USE INDEX(dateline) WHERE status=0 AND !locked AND times<10");
while($result = $_SGLOBAL['db']->fetch_array($ComplainQuery)) {
    ...
    if ($result['times'] == 3 && $result['issendmsg'] == 1 && $nowtime - $result['dateline'] > 3 * 24 * 3600) {
        ...
        addNeedSend($result,$UpUserArray2['dept_uid'], $nexttime, "条诉求待处理,最早的一条将于".fancyDate($nexttime)."上报给校长,请您安排处理.", $UpUserArray2, array($result['atuid'] => $UserArray['mobile'].',1', $UpUserArray['dept_uid'] => $UpUserArray['mobile'].',3'), 7);
